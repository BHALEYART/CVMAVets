#!/usr/bin/env node
// Builds the albums on gallery/index.html from the folders in assets/img/gallery/.
...
const p = join(root, 'gallery', 'index.html');
const html = readFileSync(p, 'utf8');
const re = /(<!-- GALLERY:ALBUMS -->)[\s\S]*?([ \t]*<!-- \/GALLERY:ALBUMS -->)/;
if (!re.test(html)) throw new Error('gallery/index.html is missing the GALLERY:ALBUMS markers');
writeFileSync(p, html.replace(re, (_, a, b) => `${a}\n${blocks.join('\n')}${blocks.length ? '\n' : ''}${b}`));
console.log(`gallery/index.html updated with ${blocks.length} album(s).`);
